...
def get_approx_coeffs(conditions):
    ...
    a = taylor_lib.series_multiply(a, taylor_lib.series_multiply(lin_term, lin_term))
...
def get_approximant(conditions):
    ...
    def theta(t):
        ...
        return np.pi + wt*(t-T_2) + ((t-T_2)**2)/(sum(A*np.exp(-n*t/np.sqrt(2)) for (n, A) in enumerate(coeffs)))
    return theta
