...
def get_approx_coeffs(conditions):
    ...
    cosine_sq = taylor_lib.series_multiply(cosine, cosine)
    a = taylor_lib.series_multiply(a, cosine_sq)
...
def get_approximant(conditions):
    ...
    def theta(t):
        ...
        return np.pi - (2*T_2*wt/np.pi)*np.cos(.5*np.pi*t/T_2)*(np.sin(.5*np.pi*t/T_2)**2) + (np.cos(.5*np.pi*t/T_2)**2)/(sum(A*np.exp(-n*t/np.sqrt(2)) for (n, A) in enumerate(coeffs)))
    return theta
