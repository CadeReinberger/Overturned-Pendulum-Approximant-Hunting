...
def get_approx_coeffs(conditions):
    ...
    a = taylor_lib.series_multiply(a, taylor_lib.series_power(tanh, 2))
...
def get_approximant(conditions):
    ...
    def theta(t):
        ...
        return np.pi + (wt*np.sqrt(2))*np.tanh((t-T_2)/np.sqrt(2)) + (np.tanh((t-T_2)/np.sqrt(2))**2)/sum(A*np.exp(-n*t/np.sqrt(2)) for (n, A) in enumerate(coeffs))
    return theta
