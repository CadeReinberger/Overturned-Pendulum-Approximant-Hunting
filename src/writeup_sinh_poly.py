...
def get_approx_coeffs(conditions):
    ...
    inside = taylor_lib.fill(-T_2, 1, conditions.N)
    mult_recip = taylor_lib.series_power(inside, -3)
    #mult_recip = taylor_lib.series_recip(taylor_lib.series_multiply(taylor_lib.series_multiply(inside, inside), inside))
...
def get_approximant(conditions):
    ...
    def theta(t):
        ...
        return np.pi  + theta_dot_tau*np.sqrt(2)*np.sinh((t-T_2)/np.sqrt(2)) + ((t-T_2)**3)*sum(a * (t ** i) for (i, a) in enumerate(coeffs))
    return theta
